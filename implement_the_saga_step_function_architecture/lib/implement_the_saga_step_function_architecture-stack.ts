...
export class ImplementTheSagaStepFunctionArchitectureStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    super(scope, id, props);
      ...
      // 3) Payment
         // 1) Flights 
    let reserveFlightLambda = createLambda(this, 'reserveFlightLambdaHandler', 'flights/reserveFlight.handler', bookingsTable);
    let confirmFlightLambda = createLambda(this, 'confirmFlightLambdaHandler', 'flights/confirmFlight.handler', bookingsTable);
    let cancelFlightLambda = createLambda(this, 'cancelFlightLambdaHandler', 'flights/cancelFlight.handler', bookingsTable);

    // 2) Hotel 
    let reserveHotelLambda = createLambda(this, 'reserveHotelLambdaHandler', 'hotel/reserveHotel.handler', bookingsTable);
    let confirmHotellambda = createLambda(this, 'confirmHotelLambdaHandler', 'hotel/confirmHotel.handler', bookingsTable);
    let cancelHotelLambda = createLambda(this, 'cancelHotelLambdaHandler', 'hotel/cancelHotel.handler', bookingsTable);

    // 3) Payment For Holiday
    let takePaymentLambda = createLambda(this, 'takePaymentLambdaHandler', 'payment/takePayment.handler', bookingsTable);
    let refundPaymentLambda = createLambda(this, 'refundPaymentLambdaHandler', 'payment/refundPayment.handler', bookingsTable);
   ...
   * @param table 
   */
  function createLambda(scope:cdk.Stack, id:string, handler:string, table:dynamodb.Table){
